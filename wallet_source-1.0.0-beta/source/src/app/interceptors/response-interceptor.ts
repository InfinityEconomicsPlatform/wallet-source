...
export class ResponseInterceptor implements HttpInterceptor {
    ...
    constructor(private http: HttpClient, private commonService: CommonService, private sessionStorageService: SessionStorageService, private peerService: PeerService, private optionsConfigurationService: OptionsConfigurationService) {
    }
        ...
        return next.handle(req)
            .map(response => {
                if (response instanceof HttpResponse) {
                    ...
                    if (url.indexOf('/api/nodes') !== -1 && Array.isArray(response.body) && response.body.length === 0) {
                ...
                if (url.indexOf('api') !== -1 && response.params && response.params.requestType && response.params.requestType !== 'getPeerState' && response.status === -1) {
                    var currentTry = _this.sessionStorageService.getFromSession(AppConstants.baseConfig.SESSION_CURRENT_TRY) || 0;
                    var maxTries = AppConstants.baseConfig.SESSION_MAX_RETRIES;
                    if (currentTry > maxTries) {
                        _this.sessionStorageService.saveToSession(AppConstants.baseConfig.SESSION_CURRENT_TRY, 0);
                        return Observable.of(response);
                    } else {
                        _this.sessionStorageService.saveToSession(AppConstants.baseConfig.SESSION_CURRENT_TRY, currentTry + 1);
