...
export class CampaignsComponent implements OnInit {
  ...
  setPage(pageInfo) {
    ...
    this.crowdfundingService.getAllCampaigns(
  ...
  openCurrencyDetails(code){
    this.router.navigate(['/crowdfunding/show-campaigns/currency-details'], { queryParams: { id:code }});
  }

  openFoundersCampaign(code){
    this.router.navigate(['/crowdfunding/show-campaigns/reserve-founders'], { queryParams: { id:code }});
  }

  openTradeDesk(value, decimals){
    this.router.navigate(['/currencies/show-currencies']);
  }

  openReserveCampaign(value, decimals, minReservePerUnitTQT, code, reserveSupply){
    DataStoreService.set('reserve-units',{id:value, decimals, minReservePerUnitTQT, code, reserveSupply})
