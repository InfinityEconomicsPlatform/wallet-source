...
export class TradeDeskComponent implements OnInit {
    ...
    getCurrency() {
        ...
        this.route.params.subscribe(params => {
            ...
            this.currenciesService.getCurrencyById(this.currencyId).subscribe((success) => {
                ...
                this.currenciesService.getSingleAccountCurrency(this.accountRs, this.currencyId).subscribe((success) => {
    ...
    getBuyOffers(pageInfo?) {
        ...
        var currencyOrders = this.currenciesService.getBuyOffers(undefined, this.currencyId, this.buyOffersPage.pageNumber * 10, ((this.buyOffersPage.pageNumber + 1) * 10) - 1);
        var currencyDetails = this.currenciesService.getCurrencyById(this.currencyId);
        ...
        Observable.forkJoin(observablesArray)
            .subscribe((successNext: any) => {
                ...
                if(pageInfo.offset == 0){
                    this.bidLength = offersResponse.offers ? offersResponse.offers.length : 0;
                }
    ...
    getSellOffers(pageInfo?) {
        ...
        var observablesArray = [];
        var currencyOrders = this.currenciesService.getSellOffers(undefined, this.currencyId, this.sellOffersPage.pageNumber * 10, ((this.sellOffersPage.pageNumber + 1) * 10) - 1);
        var currencyDetails = this.currenciesService.getCurrencyById(this.currencyId);
        observablesArray.push(currencyOrders);
        if (!this.decimals) {
            observablesArray.push(currencyDetails);
        ...
        Observable.forkJoin(observablesArray)
            .subscribe((successNext: any) => {
                ...
                if (pageInfo.offset == 0) {
                    this.askLength = offersResponse.offers ? offersResponse.offers.length : 0;
                }
    ...
    openPublishExchangeOffer(currencyId, decimals, ticker, name) {
        ...
        this.router.navigate(['publish-exchange-offer'], { relativeTo: this.route });
    }

    openPublishExchangeBuyOffer(currencyId, decimals, ticker, name) {
        DataStoreService.set('publish-exchange-buy-offer', { currencyId, decimals, ticker, name });
        this.router.navigate(['publish-exchange-buy-offer'], { relativeTo: this.route });
    }

    openPublishExchangeSellOffer(currencyId, decimals, ticker, name) {
        DataStoreService.set('publish-exchange-sell-offer', { currencyId, decimals, ticker, name });
        this.router.navigate(['publish-exchange-sell-offer'], { relativeTo: this.route });
    }
    ...
    getTradeData(trades) {
        ...
        for (var i = 0; i < optionSize; i++) {
            ...
            var x = optionSize - i;
            var price = this.quantToAmountPipe.transform(this.shareToQuantityPipe.transform(trades[i].rateTQT, this.decimals));
            var quantity = this.quantityToSharePipe.transform(trades[i].units, this.decimals);
    ...
    openCurrencyDetails(code) {
        this.router.navigate(['currency-details'], { queryParams: { id: code }, relativeTo: this.route });
    }
}
