...
export class ShufflingsComponent implements OnInit {
  ...
  filters = [
    { name: 'Active Shuffles', icon: 'fa-minus', popoverText: 'Active Shuffles', isEnabled: false },
    { name: 'All Shuffles', icon: 'fa-plus', popoverText: 'All Shuffles', isEnabled: false },
  ];
  ...
  setPage(pageInfo) {
    ...
    if (this.shufflingType == 'ALL') {
      this.shufflingService.getAllShufflings(
        startIndex,
        endIndex,
      ...
      combined.subscribe((latestValues:any) => {
        var [accountShufflingsResponse, shufflersPromiseResponse] = latestValues;
  ...
  clearAllShufflers() {
    var clearShufflerPromise =
        this.shufflingService.getAccountShufflings(this.accountId, undefined, undefined, true).subscribe( (success) => {
            ...
            if (existingShufflers.length > 0) {
                ...
                for (var i = 0; i < maxLength; i++) {
                    ...
                    if ((current.stage === 4 || current.stage === 5)) {
                        observablesArray.push(this.shufflingService.stopShuffler(current.shufflingFullHash,this.secretPhrase));
                    }
                }
                if (observablesArray.length > 0) {
                    Observable.forkJoin(observablesArray).subscribe((latestValues:any) => {
  ...
  openAccountDetails(accountID) {
    this.router.navigate(['/shuffling/show-shufflings/account-details'], { queryParams: { id: accountID } });
  }

  openShufflingDetails(shuffling){
    this.router.navigate(['/shuffling/show-shufflings/shuffling-details'], { queryParams: { id: shuffling } });
  }

  openShufflingParticipants(shuffling){
    this.router.navigate(['/shuffling/show-shufflings/shuffling-participants'], { queryParams: { id: shuffling } });
  }
  ...
  startShuffle(shufflingFullHash, amount){
    this.router.navigate(['/shuffling/show-shufflings/start-shuffling'], { queryParams: { id: shufflingFullHash } });
  }

  stopShuffle(shufflingFullHash){
    this.router.navigate(['/shuffling/show-shufflings/stop-shuffling'], { queryParams: { id: shufflingFullHash } });
  }

  joinShuffle(shufflingFullHash, amount, shuffling, holding){
    this.router.navigate(['/shuffling/show-shufflings/join-shuffling'], { queryParams: { id: shufflingFullHash, amount, shuffling, holding } });
  }
