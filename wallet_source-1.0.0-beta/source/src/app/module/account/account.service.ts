...
export class AccountService {
    ...
    getAccountDetails(accountRS) {
        let params = {
            ...
            'includeAssets': 'true', 'includeCurrencies': 'false', 'includeEffectiveBalance': 'true',
            'includeLessors': 'true',
            'account': accountRS
        };
    ...
    createTransaction(senderPublicKey, recipientRS, amount, fee, data, nonce, recipientPublicKey) {
    ...
    setBalanceLeasing(senderPublicKey, recipientRS, period, fee, data, nonce, recipientPublicKey) {
        let params = {
    ...
    blockGeneration(mode, secret, node) {
        let baseUrl = '';
        if (node !== 'LOCAL_HOST') {
            baseUrl = this.nodeService.getNodeUrl(this.optionsService.getOption('CONNECTION_MODE', ''),
        ...
        return this.http.post(baseUrl, AppConstants.accountConfig.accountEndPoint, params);
    };
    ...
    startFundingMonitor(property, amount, threshold, interval, holding, holdingType, secretPhrase, adminPassword) {
        let params = {
    ...
    stopFundingMonitor(property, holding, holdingType, account, secretPhrase, adminPassword) {
        let params = {
    ...
    getFundingMonitors(property, holding, holdingType, account, secretPhrase, adminPassword) {
        let params = {
