...
export class CreateEscrowComponent implements OnInit {
    ...
    searchAliases() {
        this.aliasesService.searchAlias(this.escrowForm.recipientRS).subscribe((success) => {
            var aliases = success.aliases ||[];
            for (var i = 0; i < aliases.length; i++) {
                var alias = aliases[i];
                if (alias.aliasName.toUpperCase() === this.escrowForm.recipientRS.toUpperCase()) {
                    ...
                    _success.subscribe(result => {
                        if (!result.errorCode) {
                            ...
                            alertFunctions.InfoAlertBox('Error',
                                'Sorry, an error occured! Reason: ' + result.errorDescription,
                                'OK',
