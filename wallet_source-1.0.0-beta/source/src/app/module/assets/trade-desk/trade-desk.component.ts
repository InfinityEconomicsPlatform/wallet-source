...
export class TradeDeskComponent implements OnInit {
    ...
    getAsset(){
        this.assetsService.getAsset(this.assetId, true).subscribe((success: any) => {
            this.assetDetails = success;
            this.decimals = success.decimals;

            this.assetsService.getAccountSingleAsset(this.accountRs, this.assetId).subscribe((success: any) => {
    ...
    buyFormOnChange() {
        this.buyOrderForm.totalPrice = this.numericalStringPipe.transform(parseFloat((this.buyOrderForm.quantity * this.buyOrderForm.price) + ''));
        if (this.buyOrderForm.price && this.buyOrderForm.quantity && this.askLength) {

            var askQuantityQnt = this.shareToQuantityPipe.transform(this.buyOrderForm.quantity, this.decimals);
    ...
    sellFormOnChange() {
        this.askOrderForm.totalPrice = this.numericalStringPipe.transform(parseFloat((this.askOrderForm.quantity * this.askOrderForm.price) + ''));
        if (this.askOrderForm.price && this.askOrderForm.quantity && this.bidLength) {
            if (this.unconfirmedQuantityQNT && this.askOrderForm.price && this.askOrderForm.quantity) {

                var sellQuantityQnt = this.shareToQuantityPipe.transform(this.askOrderForm.quantity, this.decimals);
    ...
    getSellOrders(pageInfo?){
        ...
        var assetOrders = this.assetsService.getAssetOrders(this.assetId, this.assetsService.GET_ASSET_ORDERS.ASK_ORDER,this.sellOrdersPage.pageNumber * 10,((this.sellOrdersPage.pageNumber + 1) * 10) - 1);
        ...
        Observable.forkJoin(observablesArray)
            .subscribe((successNext: any) => {
                ...
                if (this.sellOrdersPage.pageNumber === 0 && this.sellOrdersRows.length < 10) {
                    this.sellOrdersPage.totalElements = this.sellOrdersRows.length;
                } else if (this.sellOrdersPage.pageNumber > 0 && this.sellOrdersRows.length < 10) {
                    this.sellOrdersPage.totalElements = this.sellOrdersPage.pageNumber * 10 + this.sellOrdersRows.length;
    ...
    getBuyOrders(pageInfo?){
        ...
        var observablesArray = [];
        var assetOrders = this.assetsService.getAssetOrders(this.assetId, this.assetsService.GET_ASSET_ORDERS.BID_ORDER, this.buyOrdersPage.pageNumber * 10,((this.buyOrdersPage.pageNumber + 1) * 10) - 1);
        ...
        Observable.forkJoin(observablesArray)
            .subscribe((successNext: any) => {
                ...
                if (this.buyOrdersPage.pageNumber === 0 && this.buyOrdersRows.length < 10) {
                    this.buyOrdersPage.totalElements = this.buyOrdersRows.length;
                } else if (this.buyOrdersPage.pageNumber > 0 && this.buyOrdersRows.length < 10) {
                    this.buyOrdersPage.totalElements = this.buyOrdersPage.pageNumber * 10 + this.buyOrdersRows.length;
    ...
    getAssetLastTrades(pageInfo?){
        ...
        this.assetsService.getAssetLastTrades(
            ...
            .subscribe((response: any) => {
                ...
                } else if (this.tradesPage.pageNumber > 0 && this.tradesRows.length < 10) {
                    this.tradesPage.totalElements = this.tradesPage.pageNumber * 10 + this.tradesRows.length;
    ...
    getTradeData(trades){
        ...
        for (var i = 0; i < optionSize; i++) {
            ...
            var x = optionSize - i;
            var price = this.quantToAmountPipe.transform(this.shareToQuantityPipe.transform(trades[i].priceTQT, this.decimals));
            var quantity = this.quantityToSharePipe.transform(trades[i].quantityQNT, this.decimals);
    ...
    fillSellOrderForm(row){
        this.askOrderForm.price = row.priceTQT * Math.pow(10, row.decimals) / 100000000;  // this.rateTqtToPricePipe.transform(row.priceTQT, row.decimals) ;
        this.askOrderForm.quantity = this.quantityToSharePipe.transform(row.quantityQNT, row.decimals);
        this.askOrderForm.totalPrice = this.numericalStringPipe.transform(this.quantToAmountPipe.transform(row.priceTQT) * row.quantityQNT)
    }
    fillBuyOrderForm(row){
        this.buyOrderForm.price = row.priceTQT * Math.pow(10, row.decimals) / 100000000;
        this.buyOrderForm.quantity = this.quantityToSharePipe.transform(row.quantityQNT, row.decimals);
        this.buyOrderForm.totalPrice = this.numericalStringPipe.transform(this.quantToAmountPipe.transform(row.priceTQT) * row.quantityQNT);
    }
    renderChart(){
    ...
    openAccountDetails(accountID){
        this.router.navigate(['account-details'], { queryParams: { id: accountID }, relativeTo: this.route });
    }

    openAssetDetails(code){
        this.router.navigate(['asset-details'], { queryParams: { id:code },  relativeTo: this.route});
    }
